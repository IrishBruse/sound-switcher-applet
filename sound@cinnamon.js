...
class ControlButton {
  constructor(icon, tooltip, callback, small = false) {
    ...
    if (small) this.button.add_style_pseudo_class("small");
...
class VolumeSlider extends PopupMenu.PopupSliderMenuItem {
  constructor(applet, stream, tooltip, app_icon) {
    ...
    if (tooltip) this.tooltipText = tooltip + ": ";
  ...
  connectWithStream(stream) {
    if (!stream) {
      ...
      this.stream = stream;
      this.isMic =
        stream instanceof Cvc.MixerSource ||
        stream instanceof Cvc.MixerSourceOutput;
  ...
  _onValueChanged() {
    ...
    let volume =
      this._value *
      (this.isOutputSink ? this.applet._volumeMax : this.applet._volumeNorm);
    ...
    if (this._value < 0.005) {
      ...
      if (
        ...
        volume > this.applet._volumeNorm * (1 - VOLUME_ADJUSTMENT_STEP / 2) &&
        volume < this.applet._volumeNorm * (1 + VOLUME_ADJUSTMENT_STEP / 2)
      )
        volume = this.applet._volumeNorm;
    }
    ...
    if (this.stream.is_muted !== muted) this.stream.change_is_muted(muted);

    if (!this._dragging) this.applet._notifyVolumeChange(this.stream);
  }
  ...
  _onScrollEvent(actor, event) {
    ...
    if (direction == Clutter.ScrollDirection.DOWN) {
      this._value = Math.max(
        0,
        this._value -
          (VOLUME_ADJUSTMENT_STEP / this.applet._volumeMax) *
            this.applet._volumeNorm
      );
  ...
  _onKeyPressEvent(actor, event) {
    ...
    if (key === Clutter.KEY_Right || key === Clutter.KEY_Left) {
      ...
      this._value = Math.max(
        ...
        Math.min(
          this._value +
            (delta / this.applet._volumeMax) * this.applet._volumeNorm,
          1
        )
      );
      this._slider.queue_repaint();
...
class Seeker extends Slider.Slider {
  constructor(mediaServerPlayer, props, playerName) {
    ...
    this._seekChangedId = mediaServerPlayer.connectSignal(
      "Seeked",
      (id, sender, value) => {
  ...
  _updateTimer() {
    ...
    if (this.status === "Playing") {
      ...
      if (this.status === "Stopped") this._currentTime = 0;
      this._updateValue();
    }
  }
  ...
  _getCanSeek() {
    ...
    this._prop.GetRemote(
      MEDIA_PLAYER_2_PLAYER_NAME,
      "CanSeek",
      (position, error) => {
        if (!error) this._setCanSeek(position[0].get_boolean());
      }
    );
  }
  ...
  _getPosition() {
    this._prop.GetRemote(
      ...
      "Position",
      (position, error) => {
        if (!error) this._setPosition(position[0].get_int64());
      }
    );
  }
...
class Player extends PopupMenu.PopupMenuSection {
  ...
  _dbus_acquired() {
    ...
    this._prevButton = new ControlButton(
      "media-skip-backward",
      _("Previous"),
      () => this._mediaServerPlayer.PreviousRemote()
    );
    this._playButton = new ControlButton(
    ...
    this._loopButton = new ControlButton(
      "media-playlist-consecutive",
      _("Consecutive Playing"),
      () => this._toggleLoopStatus()
    );
    this.controls.add_actor(this._loopButton.getActor());

    this._shuffleButton = new ControlButton(
    ...
    this._propChangedId = this._prop.connectSignal(
      ...
      (proxy, sender, [iface, props]) => {
        ...
        if (props.CanGoNext || props.CanGoPrevious) this._updateControls();
        if (props.LoopStatus) this._setLoopStatus(props.LoopStatus.unpack());
        if (props.Shuffle) this._setShuffle(props.Shuffle.unpack());
        if (props.Identity) {
  ...
  _setMetadata(metadata) {
    ...
    } else this._artist = _("Unknown Artist");

    this.artistLabel.set_text(this._artist);

    if (metadata["xesam:album"]) this._album = metadata["xesam:album"].unpack();
    else this._album = _("Unknown Album");

    if (metadata["xesam:title"]) this._title = metadata["xesam:title"].unpack();
    else this._title = _("Unknown Title");
        ...
        if (this._trackCoverFile.match(/^http/)) {
          ...
          Util.spawn_async(
            [
              ...
              "-O",
              this._trackCoverFileTmp.get_path(),
            ],
            () => this._onDownloadedCover()
          );
        } else if (
          this._trackCoverFile.match(/data:image\/(png|jpeg);base64,/)
        ) {
          if (!this._trackCoverFileTmp)
            this._trackCoverFileTmp = Gio.file_new_tmp(
              "XXXXXX.mediaplayer-cover"
  ...
  _toggleLoopStatus() {
    ...
    this._mediaServerPlayer.LoopStatus =
      mapping[this._mediaServerPlayer.LoopStatus];
    this._setLoopStatus(this._mediaServerPlayer.LoopStatus);
  }

  _setLoopStatus(status) {
    this._loopButton.actor.visible =
      this._applet.extendedPlayerControl && this._mediaServerPlayer.LoopStatus;

    if (status === "None")
      this._loopButton.setData(
  ...
  _setShuffle(status) {
    this._shuffleButton.actor.visible =
      this._applet.extendedPlayerControl && this._mediaServerPlayer.Shuffle;

    this._shuffleButton.setData(
      "media-playlist-shuffle",
      status ? _("Shuffle") : _("No Shuffle")
    );
  ...
  destroy() {
    ...
    if (this._prop) this._prop.disconnectSignal(this._propChangedId);
...
class CinnamonSoundApplet extends Applet.TextIconApplet {
  constructor(metadata, orientation, panel_height, instanceId) {
    ...
    this.settings.bind(
      ...
      "extendedPlayerControl",
      function () {
        for (let i in this._players) this._players[i].onSettingsChanged();
      }
    );
    ...
    this._control.connect("state-changed", (...args) =>
      this._onControlStateChanged(...args)
    );

    this._control.connect("output-added", (...args) =>
      this._onDeviceAdded(...args, "output")
    );
    this._control.connect("output-removed", (...args) =>
      this._onDeviceRemoved(...args, "output")
    );
    this._control.connect("active-output-update", (...args) =>
      this._onDeviceUpdate(...args, "output")
    );

    this._control.connect("input-added", (...args) =>
      this._onDeviceAdded(...args, "input")
    );
    this._control.connect("input-removed", (...args) =>
      this._onDeviceRemoved(...args, "input")
    );
    this._control.connect("active-input-update", (...args) =>
      this._onDeviceUpdate(...args, "input")
    );

    this._control.connect("stream-added", (...args) =>
      this._onStreamAdded(...args)
    );
    this._control.connect("stream-removed", (...args) =>
      this._onStreamRemoved(...args)
    );
    ...
    this.actor.connect("scroll-event", (...args) =>
      this._onScrollEvent(...args)
    );

    this.mute_out_switch = new PopupMenu.PopupSwitchIconMenuItem(
      _("Mute output"),
      false,
      "audio-volume-muted",
      St.IconType.SYMBOLIC
    );
    this.mute_in_switch = new PopupMenu.PopupSwitchIconMenuItem(
      _("Mute input"),
      false,
      "microphone-sensitivity-muted",
      St.IconType.SYMBOLIC
    ...
    if (this.alwaysShowMuteInput) this.mute_in_switch.actor.show();
    else this.mute_in_switch.actor.hide();

    this._applet_context_menu.addMenuItem(
      new PopupMenu.PopupSeparatorMenuItem()
    );

    this._outputApplicationsMenu = new PopupMenu.PopupSubMenuMenuItem(
      _("Applications")
    );
    ...
    this._inputVolumeSection = new VolumeSlider(
      this,
      null,
      _("Microphone"),
      null
    );
    this._inputVolumeSection.connect("values-changed", (...args) =>
      this._inputValuesChanged(...args)
    );
    this._selectInputDeviceItem = new PopupMenu.PopupSubMenuMenuItem(
      _("Input device")
    );
    ...
    this._sound_settings.connect("changed::" + OVERAMPLIFICATION_KEY, () =>
  ...
  on_settings_changed() {
    ...
    else this.setAppletTextIcon();

    if (this.alwaysShowMuteInput) this.mute_in_switch.actor.show();
    else if (this._recordingAppsNum === 0) this.mute_in_switch.actor.hide();
  ...
  on_applet_removed_from_panel() {
    ...
    for (let i in this._players) this._players[i].destroy();
  ...
  _onScrollEvent(actor, event) {
    ...
    if (direction == Clutter.ScrollDirection.DOWN) {
      ...
      this._output.volume = Math.max(
        0,
        currentVolume - this._volumeNorm * VOLUME_ADJUSTMENT_STEP
      );
      if (this._output.volume < 1) {
        ...
        if (
          ...
          this._output.volume >
            this._volumeNorm * (1 - VOLUME_ADJUSTMENT_STEP / 2) &&
          this._output.volume <
            this._volumeNorm * (1 + VOLUME_ADJUSTMENT_STEP / 2)
        )
          this._output.volume = this._volumeNorm;
  ...
  _onButtonPressEvent(actor, event) {
    ...
    if (buttonId === 2) {
      if (shiftPressed) {
        if (this.middleShiftClickAction === "mute") {
          if (
            this._input &&
            this._output &&
            this._output.is_muted === this._input.is_muted
          )
          ...
          this._players[
            this._activePlayer
          ]._mediaServerPlayer.PlayPauseRemote();
      } else {
        if (this.middleClickAction === "mute") {
          ...
          this._toggle_out_mute();
        else if (this.middleClickAction === "in_mute") this._toggle_in_mute();
      ...
      return Applet.Applet.prototype._onButtonPressEvent.call(
        this,
        actor,
        event
      );
  ...
  setIcon(icon, source) {
    ...
    if (source) {
      ...
      else this._playerIcon = [icon, source === "player-path"];
    }
    ...
    if (this.playerControl && this._activePlayer && this._playerIcon[0]) {
      if (source === "output") {
        ...
        if (this.stream && !this.stream.is_muted) {
          this._iconTimeoutId = Mainloop.timeout_add_seconds(
            OUTPUT_ICON_SHOW_TIME_SECONDS,
            () => {
              this.setIcon();
            }
          );
        ...
        if (this._playerIcon[1]) this.set_applet_icon_path(this._playerIcon[0]);
        else this.set_applet_icon_symbolic_name(this._playerIcon[0]);
      }
    } else {
  ...
  setAppletIcon(player, path) {
    ...
    if (this.showalbum) {
      if (
        path &&
        player &&
        (player === true || player._playerStatus == "Playing")
      ) {
  ...
  setAppletTextIcon(player, icon) {
    this.player = player;
    if (player && player._owner != this._activePlayer) return;
  ...
  _changePlayerOwner(busName, oldOwner, newOwner) {
    if (
      this._players[oldOwner] &&
      busName == this._players[oldOwner]._busName
    ) {
  ...
  passDesktopEntry(entry) {
    ...
    for (let i = 0, l = this._knownPlayers.length; i < l; ++i) {
      if (this._knownPlayers[i] === entry) return;
    }
  ...
  _showFixedElements() {
    ...
    this._launchPlayerItem = new PopupMenu.PopupSubMenuMenuItem(
      _("Launch player")
    );
    ...
    this._chooseActivePlayerItem = new PopupMenu.PopupSubMenuMenuItem(
      _("Choose player controls")
    );
    ...
    this._outputVolumeSection.connect("values-changed", (...args) =>
      this._outputValuesChanged(...args)
    );
  ...
  _updateLaunchPlayer() {
    ...
    for (let i = 0, l = this._knownPlayers.length; i < l; ++i) {
      ...
      if (app) availablePlayers.push(app);
    }
    ...
    if (availablePlayers.length > 0) {
      for (var p = 0; p < availablePlayers.length; p++) {
        ...
        let menuItem = new MediaPlayerLauncher(
          playerApp,
          this._launchPlayerItem.menu
        );
  ...
  _updatePlayerMenuItems() {
    if (this.playerControl && this._activePlayer) {
      ...
      for (let i = 0, l = this._playerItems.length; i < l; ++i) {
        ...
        playerItem.item.setShowDot(
          playerItem.player._owner === this._activePlayer
        );
      }
  ...
  _readOutput() {
    ...
    if (this._output) {
      ...
      this._outputMutedId = this._output.connect(
        "notify::is-muted",
        (...args) => this._mutedChanged(...args, "_output")
      );
  ...
  _readInput() {
    ...
    if (this._input) {
      ...
      this._inputMutedId = this._input.connect("notify::is-muted", (...args) =>
        this._mutedChanged(...args, "_input")
      );
  ...
  _onDeviceAdded(control, id, type) {
    ...
    let selectItem =
      this["_select" + type[0].toUpperCase() + type.slice(1) + "DeviceItem"];
    selectItem.menu.addMenuItem(item);
    //show the menu if we have more than two devices
    if (selectItem.menu.numMenuItems > 1) selectItem.actor.show();
  ...
  _onDeviceRemoved(control, id, type) {
    for (let i = 0, l = this._devices.length; i < l; ++i) {
      if (this._devices[i].type === type && this._devices[i].id === id) {
        ...
        if (device.item) device.item.destroy();

        //hide submenu if showing them is unnecessary
        let selectItem =
          this[
            "_select" + type[0].toUpperCase() + type.slice(1) + "DeviceItem"
          ];
        if (selectItem.menu.numMenuItems <= 1) selectItem.actor.hide();
  ...
  _onStreamRemoved(control, id) {
    for (let i = 0, l = this._streams.length; i < l; ++i) {
      if (this._streams[i].id === id) {
        ...
        if (stream.item) stream.item.destroy();
  ...
  registerSystrayIcons() {
    for (let i = 0; i < players_with_seek_support.length; i++) {
      Main.systrayManager.registerTrayIconReplacement(
        players_with_seek_support[i],
        this.metadata.uuid
      );
    }
    for (let i = 0; i < players_without_seek_support.length; i++) {
      Main.systrayManager.registerTrayIconReplacement(
        players_without_seek_support[i],
        this.metadata.uuid
      );
    }
  }
...
function main(metadata, orientation, panel_height, instanceId) {
  return new CinnamonSoundApplet(
    metadata,
    orientation,
    panel_height,
    instanceId
